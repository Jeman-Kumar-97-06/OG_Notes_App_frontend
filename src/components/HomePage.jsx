  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br from-indigo-100 via-pink-50 to-purple-100 ${theme == 'dark' ? 'dark:from-gray-900 dark:via-gray-800 dark:to-gray-900' : ''} flex flex-col transition-colors`}>
      ...
      <section className="flex flex-col items-center justify-center text-center flex-1 px-6">
        <h2 className={`text-4xl font-bold text-indigo-800 ${theme == 'dark' ? 'dark:text-indigo-200' : ''} mb-4`}>
          Welcome to Your Mental Health Journal
        </h2>
        <p className={`text-lg text-gray-600 ${theme == 'dark'?'dark:text-gray-300':''} max-w-2xl mb-8`}>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6 p-8">
        <div className={`bg-white ${theme == 'dark' ? 'dark:bg-gray-800' : ''} p-6 rounded-2xl shadow-md flex flex-col items-center`}>
          <Notebook className={`h-10 w-10 text-indigo-600 ${theme == 'dark' ? 'dark:text-indigo-400' : ''} mb-3`} />
          <h3 className={`font-semibold text-lg mb-2 text-gray-800 ${theme == 'dark' ? 'dark:text-gray-200' : ''}`}>Daily Entries</h3>
          <p className={`text-gray-600 ${theme== 'dark' ? 'dark:text-gray-400' : ''} text-center`}>
            Write your thoughts and feelings in a private, secure space.
          </p>
        </div>

        <div className={`bg-white ${theme == 'dark' ? 'dark:bg-gray-800' : ""} p-6 rounded-2xl shadow-md flex flex-col items-center`}>
          <Smile className={`h-10 w-10 text-pink-500 ${theme == 'dark' ? 'dark:text-pink-400' : ''} mb-3`}/>
          <h3 className={`font-semibold text-lg mb-2 text-gray-800 ${theme == 'dark' ? 'dark:text-gray-200' : ''}`}>Mood Tracking</h3>
          <p className={`text-gray-600 ${theme == 'dark' ? 'dark:text-gray-400' : ''} text-center`}>
        ...
        <div className={`bg-white ${theme == 'dark' ? 'dark:bg-gray-800' : ''} p-6 rounded-2xl shadow-md flex flex-col items-center`}>
          <Calendar className={`h-10 w-10 text-purple-500 ${theme== 'dark' ? 'dark:text-purple-400' : ''} mb-3`}/>
          <h3 className={`font-semibold text-lg mb-2 text-gray-800 ${theme == 'dark' ? 'dark:text-gray-200' : ''}`}>Calendar View</h3>
          <p className={`text-gray-600 ${theme == 'dark' ? 'dark:text-gray-400' : ''} text-center`}>
